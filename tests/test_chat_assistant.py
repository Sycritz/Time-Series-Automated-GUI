...
def test_build_context_snapshot_data_loaded():
    ...
    assert "Model Order: No model fitted yet" in snapshot
    assert "Forecasting Status: Cannot generate forecasts: No model fitted yet." in snapshot
...
def test_build_context_snapshot_model_fitted_no_validation():
    ...
    assert "Validation Diagnostics: Not yet run" in snapshot
    assert "Forecasting Status: Ready to generate forecasts. Model has been fitted, but validation diagnostics have not been run yet." in snapshot
...
def test_build_context_snapshot_validation_run_passed():
    ...
    assert "Forecasting Status: Ready to generate forecasts. Model fitted and successfully validated (passed diagnostic checks)." in snapshot
...
def test_build_context_snapshot_validation_run_failed():
    ...
    assert "Forecasting Status: Ready to generate forecasts, but note that the model did not pass all validation diagnostic checks (use with caution)." in snapshot
...
def test_gemini_client_stream_chat_success(mock_client_class):
    ...
    tokens = list(client.stream_chat([{"role": "user", "content": "hi"}], "system prompt"))
