...
def test_gui_forecasting_workflow(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        axis4_validation, "run_all_diagnostics", mock_run_all_diagnostics
    )
    ...
    assert (
        "Spectral Interpretation" in insight or "Why do these forecasts look" in insight
    )
    ...
    monkeypatch.setattr(
        QFileDialog,
        "getSaveFileName",
        lambda *args, **kwargs: (csv_file, "CSV Files (*.csv)"),
    )
