...
def test_gui_gating_and_indicators():
    ...
    for i in range(1, 6):
        assert (
            window.tab_widget.isTabEnabled(i) is False
        ), f"Tab {i} should be disabled initially"

    # All indicators should be neutral
    for i in range(6):
        assert (
            window.indicators[i].color.name() == "#9ca3af"
        ), f"Indicator {i} should be neutral (#9ca3af) initially, got {window.indicators[i].color.name()}"
    ...
    assert (
        window.tab_widget.isTabEnabled(5) is True
    )  # Tab 6 enabled when model is fitted

    assert window.indicators[3].color.name() == "#16a34a"
    # Indicator 4 (Validation) must be neutral because validation_run is False
    assert (
        window.indicators[4].color.name() == "#9ca3af"
    ), f"Got validation indicator color: {window.indicators[4].color.name()}"
