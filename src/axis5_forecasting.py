...
def extend_dates(original_index: pd.Index, h: int) -> pd.Index:
    ...
    if isinstance(original_index, pd.DatetimeIndex):
        ...
        if freq is not None:
            ...
            if not diffs.empty:
                delta = diffs.median()
                return pd.DatetimeIndex([original_index[-1] + (i + 1) * delta for i in range(h)])
            else:
                return pd.DatetimeIndex([original_index[-1] + pd.Timedelta(days=i + 1) for i in range(h)])
    else:
        last_val = original_index[-1]
        try:
            step = original_index[1] - original_index[0] if len(original_index) > 1 else 1
        except Exception:
            step = 1
...
def generate_forecasts(model_result, h: int, state: AnalysisState) -> dict:
    ...
    if sigma2 is None:
        params_dict = model_result.params.to_dict() if hasattr(model_result.params, 'to_dict') else {}
    ...
    # Extend index
    orig_idx = state.original_series.index if state.original_series is not None else None
...
def undo_seasonal_diff(forecast_vals: np.ndarray, prev_series: pd.Series, D: int, s: int) -> np.ndarray:
    """
...
def get_transformation_histories(original_series: pd.Series, transformations: list[dict]) -> list[pd.Series]:
    """
...
def generate_spectral_insight(state: AnalysisState) -> str:
    ...
    if total_d > 0:
        ...
        lines.append(f"- The model requires differencing of total order {total_d}. This implies that the theoretical spectrum $f_X(\\lambda)$ has an infinite singularity at frequency $\\lambda = 0$ (unbounded spectrum).")
        if total_d == 1:
            lines.append("- For a random-walk-like process ($d+D=1$), the forecast function converges to a flat line equal to the last observation (once back-transformed).")
            lines.append("- The prediction intervals expand proportional to $\\sqrt{h}$, reflecting a constant rate of uncertainty accumulation.")
        else:
            lines.append(f"- For higher integration orders ($d+D={total_d}$), the forecast function asymptotically follows a polynomial trend of degree {total_d - 1} as $h \\to \\infty$.")
            lines.append("- The prediction intervals flare out very rapidly due to the accumulation of integrated noise.")
    else:
        lines.append("**Stationary ARMA Process (d=0, D=0):**")
        lines.append("- The theoretical spectrum $f_X(\\lambda)$ is continuous and bounded, indicating no unit roots or integration.")
        lines.append("- As a result, the forecast function decays geometrically and reverts to the process mean as $h \\to \\infty$.")
        if p > 0:
            lines.append(f"- The speed of reversion is governed by the modulus of the largest AR roots. Moduli closer to 1.0 result in slower decay.")
    ...
    if is_seasonal and s > 1:
        ...
        lines.append(f"- The model captures seasonal dynamics, corresponding to distinct spectral peaks at the seasonal frequency $\\lambda_s = 2\\pi/{s} \\approx {lambda_s:.4f}$ radians and its harmonics.")
        lines.append(f"- In the time domain, these peaks translate directly to a persistent, quasi-periodic seasonal oscillation in the forecast function with a period of {s} steps.")
            ...
            try:
                ...
                if complex_roots:
                    ...
                    if modulus > 0.7:
                        ...
                        lines.append(f"- The autoregressive polynomial has complex conjugate roots with modulus {modulus:.3f} and frequency {phase:.4f} radians.")
                        lines.append(f"- This creates a peak in the theoretical spectrum $f_X(\\lambda)$ at $\\lambda \\approx {phase:.4f}$, causing the forecast function to exhibit a decaying quasi-periodic cycle of period $\\approx {period:.2f}$ steps.")
            except Exception:
                pass
