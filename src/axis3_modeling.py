...
def compute_acf_pacf(series: pd.Series, nlags: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    if gamma0 == 0:
        ...
        acf_vals = np.array(
            [
                sample_autocovariance(clean_series, h) / gamma0
                for h in range(actual_lags + 1)
            ]
        )
...
def suggest_model_from_acf_pacf(
    acf_vals: np.ndarray, pacf_vals: np.ndarray, n: int
) -> dict:
    ...
    sig_pacf = [
        lag for lag in range(1, len(pacf_vals)) if abs(pacf_vals[lag]) > threshold
    ]

    explanation = []
    explanation.append(
        f"ACF/PACF Analysis (N={n}, 95% Confidence Threshold={threshold:.4f}):"
    )
