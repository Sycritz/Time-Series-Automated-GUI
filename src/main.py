...
from PySide6.QtWidgets import (QApplication, QComboBox, QDoubleSpinBox,
                               ...
                               QPushButton, QSpinBox, QSplitter, QTabBar,
                               QTableView, QTableWidget, QTableWidgetItem,
                               QTabWidget, QTextBrowser, QVBoxLayout, QWidget)

from chat_assistant import ChatPanel
from state import AnalysisState
from widgets import PlotWidget, StatusIndicator
...
class DataLoadTab(BaseTab):
    def __init__(self, main_window, parent=None):
        ...
        self.control_layout.addWidget(
            QLabel("Select Time Series CSV:", self.control_panel)
        )
        ...
        self.control_layout.addWidget(
            QLabel("Missing Values Imputation:", self.control_panel)
        )
        self.impute_combo = QComboBox(self.control_panel)
        self.impute_combo.addItems(
            ["Forward Fill", "Linear Interpolation", "Mean Imputation"]
        )
        ...
        self.control_layout.addWidget(
            QLabel("Rolling Window size:", self.control_panel)
        )
        ...
        if file_path:
            ...
            try:
                ...
                QMessageBox.critical(
                    self, "Export Error", f"Failed to export table: {str(e)}"
                )
    ...
    def on_columns_selected(self):
        ...
        if time_col == val_col:
            QMessageBox.critical(
                self,
                "Invalid Selection",
                "Time Column and Value Column must be different.",
            )
            return
    ...
    def impute_data(self):
        if self.raw_series is None:
            QMessageBox.warning(
                self, "No Data Selected", "Please select Time and Value columns first."
            )
            return
        ...
        try:
            ...
            imputed_vals, n_imputed = impute_series(
                self.raw_series.values, method=mapped_method
            )
            imputed_series = pd.Series(
                imputed_vals, index=self.raw_series.index, name=self.raw_series.name
            )
            pct_missing = (
                (n_imputed / len(self.raw_series)) * 100.0
            ...
            if pct_missing > 5.0:
                self.warning_label.setText(
                    f"Warning: {pct_missing:.1f}% missing values imputed via {method}."
                )
    ...
    def run_adf(self):
        ...
        try:
            ...
            crit_str = "\n".join(
                [f"  {k}: {v:.4f}" for k, v in res["critical_values"].items()]
            )
...
class TransformTab(BaseTab):
    def __init__(self, main_window, parent=None):
        ...
        self.control_layout.addWidget(
            QLabel("Differencing Orders:", self.control_panel)
        )
        ...
        self.control_layout.addWidget(
            QLabel("ADF Test (Transformed):", self.control_panel)
        )
        self.adf_results_browser = QTextBrowser(self.control_panel)
        self.adf_results_browser.setFontFamily("Courier New")
        self.adf_results_browser.setMinimumHeight(150)
    ...
    def auto_optimize_lambda(self):
        ...
        if original_series is None:
            QMessageBox.warning(
                self, "No Data Loaded", "Please load a dataset on Tab 1 first."
            )
            return
    ...
    def apply_transformations(self):
        ...
        try:
            from axis1_preprocessing import (apply_box_cox, apply_differencing,
                                             run_adf_test)
            ...
            try:
                ...
                self.adf_results_browser.setText(
                    f"ADF Test could not be run: {adf_err}"
                )
    ...
    def plot_frequency_response(self):
        ...
        magnitude = (2.0 * np.abs(np.sin(omega / 2.0))) ** (2 * d) * (
            2.0 * np.abs(np.sin(s * omega / 2.0))
        ) ** (2 * D)
        ...
        if D > 0 and s > 1:
            ...
            while h * w_seas <= np.pi:
                ax.axvline(
                    ...
                    linestyle=":",
                    alpha=0.6,
                    label="Seasonal Harmonics" if h == 1 else "",
                )
                h += 1
...
class SpectralTab(BaseTab):
    def __init__(self, main_window, parent=None):
        ...
        self.control_layout.addWidget(
            QLabel("Smoothed Estimator Window:", self.control_panel)
        )
    ...
    def estimate_spectrum(self):
        ...
        if series is None:
            QMessageBox.warning(
                self,
                "No Stationary Data",
                "Please apply transformations on Tab 2 first.",
            )
            return
        ...
        if len(clean_series) < 3:
            QMessageBox.warning(
                self,
                "Insufficient Data",
                "The stationary series must contain at least 3 points.",
            )
            return
        ...
        try:
            ...
            raw_freqs, raw_pgram = compute_periodogram(
                clean_series, taper_arg, taper_pct
            )
            smooth_freqs, smooth_val, ci_lower, ci_upper = smooth_spectrum(
                clean_series, window, M
            )
    ...
    def detect_cycles_click(self):
        if (
            self.smooth_freqs is None
            or self.smooth_val is None
            or self.ci_upper is None
        ):
            QMessageBox.warning(
                self, "No Spectrum Estimated", "Please click 'Estimate Spectrum' first."
            )
            return
        ...
        try:
            ...
            cycles = detect_cycles(
                self.smooth_freqs, self.smooth_val, self.ci_upper, nu_val
            )
            ...
            if not cycles:
                ...
                for c in cycles:
                    sig_str = (
                        "<font color='#16A34A'><b>Yes</b></font>"
                        if c["significant"]
                        else "No"
                    )
    ...
    def export_cycles_csv(self):
        ...
        if file_path:
            ...
            try:
                ...
                df_cycles["frequency_cycles_per_sample"] = df_cycles["frequency"] / (
                    2.0 * np.pi
                )
                df_cycles.to_csv(file_path, index=False)
            except Exception as e:
                QMessageBox.critical(
                    self, "Export Error", f"Failed to export cycles: {str(e)}"
                )
    ...
    def update_parametric_spectrum_overlay(self, ax=None, is_period=False):
        ...
        param_freqs, param_spec = compute_parametric_spectrum(
            ar_coeffs, ma_coeffs, sigma2, n_points=512
        )
        ...
        ax.plot(
            ...
            linewidth=2.5,
            linestyle="--",
            label="Parametric ARMA Spectrum",
        )
...
class ModelingTab(BaseTab):
    def __init__(self, main_window, parent=None):
        ...
        self.control_layout.addWidget(
            QLabel("Model Heuristic Suggestions:", self.control_panel)
        )
    ...
    def plot_acf_pacf(self):
        ...
        if series is None:
            QMessageBox.warning(
                self, "No Data", "Please load and transform the data first."
            )
            return
        ...
        for ax, title, vals in zip(
            [self.acf_ax, self.pacf_ax], ["ACF", "PACF"], [acf_vals, pacf_vals]
        ):
            ax.tick_params(colors="#1F2937")
            ax.xaxis.label.set_color("#1F2937")
            ax.yaxis.label.set_color("#1F2937")
    ...
    def on_hover(self, event):
        if (
            getattr(self, "acf_vals", None) is None
            or getattr(self, "pacf_vals", None) is None
        ):
            return
    ...
    def suggest_model_spectral(self):
        ...
        explanation = []
        explanation.append(
            f"Spectral Suggestion Analysis (Seasonal Period s={seasonal_period}):"
        )

        significant_cycles = [c for c in detected_cycles if c.get("significant", False)]
        explanation.append(
            f"- Total detected significant cycles: {len(significant_cycles)}"
        )
        for i, c in enumerate(significant_cycles[:5]):
            explanation.append(
                f"  Cycle {i+1}: Period = {c['period']:.2f} samples (frequency = {c['frequency']:.4f})"
            )
    ...
    def on_grid_search_finished(self, df):
        ...
        if df.empty:
            QMessageBox.critical(
                self, "Grid Search Failed", "Grid search did not find any valid models."
            )
            self.export_models_btn.setEnabled(False)
            return
    ...
    def select_model(self, row_data):
        ...
        try:
            ...
            QMessageBox.information(
                self,
                "Model Selected",
                f"Successfully fitted and selected {row_data['Model']}.",
            )
        except Exception as e:
            QMessageBox.critical(
                self, "Fitting Error", f"Failed to fit model: {str(e)}"
            )
    ...
    def get_model_params_dict(self, res):
        ...
        return {"ar": ar_coeffs, "ma": ma_coeffs, "sigma2": sigma2}
    ...
    def display_parameters(self, res):
        ...
        for idx, name in enumerate(params.index):
            ...
            self.param_table.setItem(
                idx, 2, QTableWidgetItem(f"{se:.4f}" if not np.isnan(se) else "N/A")
            )
            self.param_table.setItem(
                idx, 3, QTableWidgetItem(f"{z:.4f}" if not np.isnan(z) else "N/A")
            )
            self.param_table.setItem(
                idx, 4, QTableWidgetItem(f"{p:.4f}" if not np.isnan(p) else "N/A")
            )
        ...
        loglik = res.llf
        self.sigma2_label.setText(
            f"Estimated σ̂²: {sigma2:.4f}"
            if not np.isnan(sigma2)
            else "Estimated σ̂²: N/A"
        )
    ...
    def export_models_csv(self):
        ...
        if file_path:
            ...
            try:
                ...
                QMessageBox.critical(
                    self, "Export Error", f"Failed to export models: {str(e)}"
                )

    def export_params_csv(self):
        if (
            getattr(self, "fitted_params_df", None) is None
            or self.fitted_params_df.empty
        ):
            return
        file_path, _ = QFileDialog.getSaveFileName(
        ...
        if file_path:
            ...
            try:
                ...
                QMessageBox.critical(
                    self, "Export Error", f"Failed to export parameters: {str(e)}"
                )
...
class ValidationTab(BaseTab):
    ...
    def run_diagnostics(self):
        ...
        if not state.model_fitted or state.residuals is None:
            QMessageBox.warning(
                self, "No Fitted Model", "Please fit and select a model first on Tab 4."
            )
            return
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Diagnostic Error", f"Failed to run diagnostics: {str(e)}"
            )
            return
    ...
    def draw_top_plots(self, residuals, std_residuals, results):
        ...
        if len(C_omega) > 0:
            ...
            ax.plot(
                cp_freqs, ks_up, color=fail_color, linestyle="--", label="95% KS Bound"
            )
    ...
    def draw_bottom_plots(self, std_residuals, results):
        ...
        if np.any(valid_indices):
            colors = [
                accent_color if p >= 0.05 else fail_color
                for p in lb_pvals[valid_indices]
            ]
            ax.bar(
                lags[valid_indices],
                lb_pvals[valid_indices],
                color=colors,
                alpha=0.8,
                edgecolor="#D1D5DB",
            )
        ...
        if len(f_hat) > 0:
            ...
            ax.fill_between(
                sp_freqs, sp_lo, sp_up, color=accent_color, alpha=0.15, label="95% CI"
            )
...
class ForecastingTab(BaseTab):
    ...
    def on_generate_clicked(self):
        ...
        if not state.model_fitted or state.fitted_model is None:
            QMessageBox.warning(
                self,
                "No Model",
                "No fitted model found. Please fit a model first on Tab 4.",
            )
            return
        ...
        try:
            from axis5_forecasting import (back_transform, generate_forecasts,
                                           generate_spectral_insight)
            ...
            QMessageBox.critical(
                self, "Forecasting Error", f"Failed to generate forecasts: {str(e)}"
            )

    def update_view(self):
        if (
            not hasattr(self, "stationary_forecasts")
            or self.stationary_forecasts is None
        ):
            return
        ...
        plot_lower_50 = np.concatenate(
            [[history_series.iloc[-1]], forecasts["lower_50"]]
        )
        plot_upper_50 = np.concatenate(
            [[history_series.iloc[-1]], forecasts["upper_50"]]
        )
        plot_lower_80 = np.concatenate(
            [[history_series.iloc[-1]], forecasts["lower_80"]]
        )
        plot_upper_80 = np.concatenate(
            [[history_series.iloc[-1]], forecasts["upper_80"]]
        )
        plot_lower_95 = np.concatenate(
            [[history_series.iloc[-1]], forecasts["lower_95"]]
        )
        plot_upper_95 = np.concatenate(
            [[history_series.iloc[-1]], forecasts["upper_95"]]
        )
        ...
        # Plot point forecast
        ax.plot(
            plot_dates, plot_point, color="#2563EB", label="Forecast", linewidth=2.0
        )
        ...
        ax.set_title(
            f"Forecast Horizon h={len(forecasts['point'])} ({scale_label})",
            color="#1F2937",
            fontsize=12,
            fontweight="bold",
        )
    ...
    def export_csv(self):
        if not hasattr(self, "current_forecasts") or self.current_forecasts is None:
            QMessageBox.warning(
                self, "No Forecasts", "Please generate forecasts first."
            )
            return
        ...
        if file_path:
            ...
            try:
                ...
                QMessageBox.information(
                    self,
                    "Export Successful",
                    f"Forecasts successfully exported to {file_path}",
                )
            except Exception as e:
                QMessageBox.critical(
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        for i, (tab, name) in enumerate(zip(self.tabs, tab_names)):
            ...
            self.tab_widget.tabBar().setTabButton(
                i, QTabBar.ButtonPosition.LeftSide, indicator
            )
    ...
    def update_ui_from_state(self):
        ...
        tab6_enabled = (
            self.state.model_fitted
        )  # ponytail: allow forecast for any fitted model; warning shown if not validated
    ...
    def closeEvent(self, event):
        try:
            ...
            if (
                hasattr(modeling_tab, "worker")
                and modeling_tab.worker
                and modeling_tab.worker.isRunning()
            ):
