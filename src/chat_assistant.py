...
from PySide6.QtWidgets import (
    QWidget, QDialog, QLineEdit, QTextBrowser, QPushButton,
...
def build_context_snapshot(state, current_tab_index: int) -> str:
    ...
    lines.append(f"  Box-Cox Lambda: {state.box_cox_lambda if state.box_cox_lambda is not None else 'None'}")
    lines.append(f"  Differencing Order (d): {state.diff_order_d}")
    lines.append(f"  Seasonal Differencing Order (D): {state.diff_order_D} (period s = {state.seasonal_period_s})")
    ...
    if state.transformations:
        ...
        for t in state.transformations:
            ...
            if t_type == "boxcox":
                ...
                trans_log.append(f"Seasonal Differencing (D = {t.get('D')}, s = {t.get('s')})")
            else:
    ...
    if state.validation_run:
        if state.residuals is not None and state.model_order is not None:
            ...
            try:
                ...
                lines.append(f"  Overall Verdict: {'PASS' if state.validation_passed else 'FAIL'}")
            except Exception as e:
                lines.append(f"Validation Diagnostics: Error running validation: {str(e)}")
        else:
            lines.append("Validation Diagnostics: Run flagged but residuals or model order missing")
    else:
...
class GeminiClient:
    ...
    def stream_chat(self, messages: list[dict], system_prompt: str = None) -> typing.Generator[str, None, None]:
        if not self.api_key:
...
class StreamWorker(QThread):
    ...
    def __init__(self, client: GeminiClient, prompt: str, history: list[dict], system_prompt: str, parent=None):
...
class ChatPanel(QWidget):
    ...
    def send_message(self):
        ...
        if not self.client.api_key:
            self.history.append({"role": "error", "content": "Please configure your Gemini API key via the gear icon in the header."})
            self._update_chat_display()
            return
        ...
        self.worker = StreamWorker(self.client, prompt, self.history[:-1], system_prompt, self)
