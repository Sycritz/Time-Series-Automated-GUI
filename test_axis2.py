...
import pandas as pd
import pytest
from axis2_spectral import (
    compute_periodogram, smooth_spectrum,
    compute_parametric_spectrum, detect_cycles
)
...
def test_compute_periodogram_demean_and_shapes():
    ...
    freqs_empty, pgram_empty = compute_periodogram(empty_series, taper=None, taper_pct=0.1)
...
def test_compute_periodogram_tapers():
    ...
    freqs, pgram = compute_periodogram(small_series, taper='Cosine Bell', taper_pct=0.25)
...
def test_smooth_spectrum_windows_and_ci():
    ...
    empty_series = pd.Series([], dtype=float)
    freqs, f_hat, ci_lower, ci_upper = smooth_spectrum(empty_series, window='Daniell', M=10)
    assert len(f_hat) == 512
    assert np.all(f_hat == 0)
...
def test_compute_parametric_spectrum():
    ...
    expected_val = (sigma2 / (2.0 * np.pi)) * ((1.0 + ma[0]) ** 2) / ((1.0 - ar[0]) ** 2)
...
def test_detect_cycles():
    ...
    assert detect_cycles(np.array([1, 2]), np.array([1.0, 1.0]), np.array([2.0, 2.0])) == []
